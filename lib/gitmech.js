...
module.exports = Gitmech = (function() {
  ...
  return {
    ...
    hashes: function(path, howMany, callback) {
      ...
      gitExec(["log", "-" + howMany, "--reverse", "--no-notes", "--pretty=format:%h", "--", docSubdir + path], function(err, data) {
    ...
    lastMessage: function(path, revision, callback) {
      gitExec(["log", "-1", revision, "--no-notes", "--pretty=format:%s", "--", docSubdir + path], function(err, data) {
